...
for i in range(1,41):
    ...
    clas = random.choice(['LMV','HMV','LCV','HCV'])
    unla = random.randrange(500,800,50)
  
    z = Rto(id = i,Vehicle_Number = v_no,Dimension_Height = dh,Dimension_Length = dl,Dimension_Breadth = db,Pollution = poll,Fitness = fit,Permit= per,Axle= ax,Vehicle_Class = clas,Registration_Date = reg,Chassis_Number = ch,Engine_Number = en,Vehicle_Fuel_Type = fuel,Engine_Cubic_Capacity = engcc,Tax = tax,Insurance = ins,National_Permit  = naper,Authorization = aut,Unladen_Weight = unla, Vehicle_Type = vt)
    z.save()
